...
async def cmd_leagues(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton(name, callback_data=f"league_{lid}")]
        for name, lid in TOP_LEAGUES.items()
    ]
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("league_"):
        ...
        league_name = next((n for n, i in TOP_LEAGUES.items() if i == league_id), "")
        ...
        if fixtures:
            keyboard = [
                [InlineKeyboardButton(
                    ...
                    callback_data=f"fixture_{f['fixture']['id']}_{league_id}"
                )]
                for f in fixtures[:8]
            ]
    ...
    # ── Fixture → full analysis ────────────────────────────────────
        ...
        await query.edit_message_text(
            ...
            "_(Fetching H2H, form, odds, Polymarket — may take ~10s)_",
            parse_mode="Markdown",
        )
        ...
        try:
            ...
            await query.edit_message_text(
                "⚠️ Analysis failed. API may be rate-limited. Try again shortly."
            )
    ...
    # ── Team → upcoming fixtures ───────────────────────────────────
        ...
        fixtures = get_fixtures_by_team(team_id, next=5)
        if not fixtures:
        ...
        keyboard = [
            [InlineKeyboardButton(
                f"{f['teams']['home']['name']} vs {f['teams']['away']['name']}",
                callback_data=f"fixture_{f['fixture']['id']}_0"
            )]
            for f in fixtures[:5]
        ]
        await query.edit_message_text(
...
async def handle_unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Hmm, I didn't understand that. Use /help to see available commands."
    )
